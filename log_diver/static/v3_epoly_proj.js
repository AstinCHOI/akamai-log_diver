...
google.maps.Polygon.prototype.GetPointsAtDistance = function(metres) {
  ...
  for (var i=1; (i < this.getPath().getLength()); i++) {
    ...
    while (dist > next) {
      // Project 
      var p1= projection.fromLatLngToPoint(this.getPath().getAt(i-1));
      var p2= projection.fromLatLngToPoint(this.getPath().getAt(i));
      var m = (next-olddist)/(dist-olddist);
      points.push(projectionToLatLng(new google.maps.Point( p1.x + (p2.x-p1.x)*m, p1.y + (p2.y-p1.y)*m)));
      next += metres;    
    }
  }
  return points;
